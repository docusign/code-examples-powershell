...
1. To configure the launcher, create a copy of the file config/settings.example.json and save the copy as config/settings.json.
   ...
   1. Set a name and email for the signer. In settings.json, save an email address as `SIGNER_EMAIL` and a name as `SIGNER_NAME`.  **Note:** Protect your personal information. Please make sure that settings.json will not be stored in your source code repository.
1. Run the launcher:
